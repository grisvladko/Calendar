...
struct Calendars: Codable {
    ...
    func widthAndXForAllDay(date:Date, event: Event) -> (width: CGFloat, x: CGFloat){
        ...
        for calendar in calendars {
            for event in calendar.events {
                ...
                if event.allDay ?? false && Calendar.current.compare(date, to: d, toGranularity: .day) == .orderedSame{
                    e.append(event)
                }
            }
        }
        var x: CGFloat = 0
        for i in 0..<e.count {
            if e[i].isEqual(event: event) {
                x = CGFloat(50) + CGFloat(i) * (CGFloat(UIScreen.main.bounds.width - 50) / CGFloat(e.count))
            }
        }
        
        let width = e.count > 1 ? (UIScreen.main.bounds.width - 50) / CGFloat(e.count) : UIScreen.main.bounds.width
        
        return (width: width, x: x)
    }
    
    func widthForEvent(date: Date, event: Event) -> (width: CGFloat, x: CGFloat) {
        var e: [Event] = []
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd HH:mm:ss z"
        ...
        for calendar in calendars {
            for event in calendar.events {
                let d = formatter.date(from: event.start)!
                print("\(d)  \(date)")
                if Calendar.current.compare(date, to: d, toGranularity: .minute) == .orderedSame && !(event.allDay ?? false){
                    e.append(event)
                }
            
                let difference = Calendar.current.dateComponents([.minute], from: date, to: d)
                if (difference.minute! == 5 || difference.minute! == 10 ||   difference.minute! == -5 || difference.minute! == -10) && !(event.allDay ?? false){
                    e.append(event)
                }
            }
        }
        ...
        for i in 0..<e.count {
            if e[i].isEqual(event: event) {
                x = CGFloat(40) + CGFloat(i) * (CGFloat(UIScreen.main.bounds.width - 40) / CGFloat(e.count))
            }
        }
        
        let width = e.count > 1 ? (UIScreen.main.bounds.width - 40) / CGFloat(e.count) : UIScreen.main.bounds.width
        
        return (width: width, x: x)
    }
}

//each calendar holds events and other data
struct MyCalendar: Codable, Equatable {
    ...
    func isEqual(calendar: MyCalendar) -> Bool {
...
struct Event: Codable, Equatable {
    ...
    init() {
        self.init(title: "", location: "", start: "", end: "", alert: "", calendar: "",allDay: nil,timeRepeat: nil, secondAlert: nil, URLS: nil, notes: nil)
    }
    
    init(title: String, location: String, start: String, end: String, alert: String, calendar: String, allDay: Bool? ,timeRepeat: Int?, secondAlert: String?, URLS: String?, notes: String?) {
        self.title = title
...
open class CheckBox: UIControl {
    ...
    var startTintColor: UIColor = #colorLiteral(red: 0, green: 0.3285208941, blue: 0.5748849511, alpha: 1)
    ...
    open override func draw(_ rect: CGRect) {
        ...
        context.setStrokeColor(self.isChecked ? checkedBorderColor.cgColor : startTintColor.cgColor)
